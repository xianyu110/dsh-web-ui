...
- **Master switch** (settings → Web UI Plugins → Right panel): `enabled`, on by default. Turn it off to unmount both panels, hide the floating expand button, and stop registering the `/aionui-panel/*` routes (workspace fs watching and git polling stop with them) — useful when another sidebar plugin owns the workspace chrome; the rest of the family bundle keeps working.
